...
class vriUVcDisp extends vriDisplay
{
   ...
   public vriUVcDisp(int x, int y, int w, int h, vriObservatory o, vriAuxiliary a)
   {
      super(x, y, w, h);
      obs = o;
      aux = a;
   ...
   public void addTracks()
   {
      ...
      for(int i = 0; i < (obs.ant.length-1); i++)
      {
         for(int j = i+1; j < (obs.ant.length); j++)
         {
            ...
            //        plotBaseline(bx, by, bz, sd, cd, s, g);
...
public void paint(Graphics g)
{
   ...
   for(int i = 0; i < 25; i++)
   {
      int pos = (int) ((double)i * grid_scale);
      g.drawLine(-pos, -mx, -pos, mx);
      g.drawLine(pos, -mx, pos, mx);
   }
   for(int i = 0; i < 25; i++)
   {
      int pos = (int) ((double)i * grid_scale);
      g.drawLine(-mx, -pos, mx, -pos);
      g.drawLine(-mx, pos, mx, pos);
   }
   */
   ...
   for(int i = 0; i < (obs.ant.length-1); i++)
   {
      for(int j = i+1; j < (obs.ant.length); j++)
      {
         ...
         // Now do old analytical calculation
         g.setColor(Color.white);

         tw = (int)( 2.0*Math.sqrt(bx*bx + by*by) );
         th = (int) Math.abs( sd*2.0*Math.sqrt(bx*bx + by*by) );
         tx = (int)( -(double)tw/2.0 );
         ty = (int)( -(double)th/2.0 + bz*cd );
         double h = aux.ha1;
         double d = aux.dec;
         u = bx*Math.sin(h) + by*Math.cos(h);
         v = by*Math.sin(h) - bx*Math.cos(h);
         int ea = (int)(90.0 - Math.atan2(u,v) * 180.0 / Math.PI);
         u = bx*Math.sin(-h) + by*Math.cos(-h);
         v = by*Math.sin(-h) - bx*Math.cos(-h);
         int sa = (int)(90.0 - Math.atan2(u,v) * 180.0 / Math.PI);
         g.drawArc(tx, ty, tw, th, sa, ea-sa);
         ty = (int)( -(double)th/2.0 - bz*cd );
         g.drawArc(tx, ty, tw, th, sa+180, ea-sa);
...
public float[] uvCoverage(int size)
{
   ...
   // Initialise the uvcoverage grid function
   for(int i = 0; i < uvcov.length; i++)
      uvcov[i] = 0;
   ...
   if(!(colour.equals(Color.black)))
   {
      for(int i = 0; i < accum.num_tracks; i++)
      {
         applyUV(accum.bx[i], accum.by[i], accum.bz[i], accum.h1[i], accum.h2[i], sd, cd, s, size, uvcov);
      }
   }
